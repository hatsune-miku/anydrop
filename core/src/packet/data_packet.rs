...
impl DataPacket {
    pub fn new(magic_number: u16, data: &Vec<u8>) -> DataPacket {
        DataPacket {
            magic_number,
            data: data.clone(),
        }
...
impl Serialize<Vec<u8>, DataPacketError> for DataPacket {
    ...
    fn deserialize(data: &Vec<u8>) -> Result<Self, DataPacketError>
    where
        Self: Sized,
    {
        ...
        let ret = DataPacket::new(magic_number, &wrapping_data);
