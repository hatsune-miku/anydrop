...
pub fn handle(context: HandlerContext) -> ConnectionControl {
    let packet = match FileReceiveResponsePacket::deserialize(context.packet().data()) {
        ...
        Err(e) => {
            warn!(
                "Failed to deserialize file receive response packet ({:?}).",
                e
            );
    ...
    info!(
        "Received file receive response packet from {}.",
        context.socket_addr()
    );

    let update_status = |status: FileSendingStatus| {
        (context.data_service_context().file_sending_callback())(
            &FileSendingPacket::new(packet.file_id(), 0, packet.file_size(), status),
            None,
        );
    };
    ...
    let mut session = |dt: &mut DataTransmit,
     ...
     -> Result<(), io::Error> {
        ...
        match file.seek(io::SeekFrom::Start(offset)) {
            Ok(n) => {
                if n != offset {
                    let error = io::Error::new(io::ErrorKind::Other, "Wrong seek position.");
        ...
        loop {
            ...
            let data_packet = DataPacket::new(
                MagicNumbers::FilePart.value(),
                &file_part_packet.serialize(),
            );

            // Send.
            if let Err(e) =
                dt.send_data_progress_with_retry(&data_packet.serialize(), |bytes_written_total| {
    ...
    if let Err(e) = DataService::data_session(
        &peer,
        context.data_service_context().port(),
