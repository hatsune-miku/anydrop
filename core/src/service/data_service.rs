...
use crate::service::handler::{image_packet_handler, text_packet_handler};
...
pub type OnPacketReceivedFunctionType<T, R> =
    Arc<Box<dyn (Fn(&T, Option<&Peer>) -> R) + Send + Sync>>;
...
impl DataService {
    ...
    where
        F: FnMut(&mut DataTransmit, &mut State) -> Result<(), io::Error>,
    {
        let mut tries = 0;
        ...
        Err(io::Error::new(
            io::ErrorKind::Other,
            "Failed to establish data session.",
        ))
    }
    ...
    fn dispatch_data_packet(
        ...
        match MagicNumbers::from(packet.magic_number()) {
            ...
            Some(MagicNumbers::Image) => image_packet_handler::handle(context),
            None => {
    ...
    fn handle_peer(stream: TcpStream, context: DataServiceContext) {
        ...
        loop {
            ...
            trace!(
                "Received data packet from {}, magic_nubmer={}.",
                socket_addr,
                data_packet.magic_number()
            );
