...
internal object TextWire {
  ...
  fun encode(text: String): ByteArray {
    val bytes = text.toByteArray(Charsets.UTF_8); require(bytes.isNotEmpty() && bytes.size <= 65535) { "文本必须为 1 到 65535 字节" }
  ...
  fun decode(bytes: ByteArray): String {
    ...
    require(count in 0..65535 && length == count + 6 && bytes.size == length + 12 && shortLE(bytes, bytes.size - 2) == length / 2)
